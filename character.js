    ...
    performAction() {
        ...
        switch (this.action.type) {
            ...
            case 'SOCIALIZE':
                ...
                // Per-character throttle (8s) — prevents greeting sound on every re-entry
                {
                    const _now = performance.now();
                    if (!this._lastSocialSoundAt || (_now - this._lastSocialSoundAt) > 8000) {
                        playSound('social');
                        this._lastSocialSoundAt = _now;
                    }
                }
                // パートナーも同時に社交状態にする
...
// ...existing code...
    ...
    update(deltaTime, isNight, camera) {
        ...
        if (this.needs.hunger <= 15 && (this.fatReserve || 0) > 0) {
            ...
            this.needs.hunger -= _rawHungerDrain;
        }
